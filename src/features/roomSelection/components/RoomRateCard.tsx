...
type Props = { rate: RoomRatePlan; recommended?: boolean; onSelect: (rate: RoomRatePlan) => void };
...
export function RoomRateCard({ rate, recommended = false, onSelect }: Props) {
  ...
  return (
    <article className="rounded-2xl border border-border bg-card p-3.5 shadow-sm">
      <div className="flex items-start gap-3">
        <span className={`mt-1 flex size-5 shrink-0 items-center justify-center rounded-full border-2 ${recommended ? "border-primary" : "border-border"}`}>
          {recommended ? <span className="size-2.5 rounded-full bg-primary" /> : null}
        </span>
        <div className="min-w-0 flex-1">
          {recommended ? <span className="mb-1.5 inline-flex rounded-full bg-secondary px-2 py-1 text-[10px] font-bold text-primary">Most Preferred</span> : null}
          ...
          <div className="mt-3 flex items-end justify-between gap-3 border-t border-border pt-3">
            ...
            <div className="text-right">
              ...
              <button type="button" onClick={() => onSelect(rate)} className="h-9 rounded-xl bg-primary px-4 text-xs font-bold text-primary-foreground shadow-sm transition active:scale-95">Select</button>
            </div>
          </div>
        </div>
      </div>
